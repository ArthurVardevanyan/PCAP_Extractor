...
def IpGeoL(ip_address):  # https://ipinfo.io/
    access_token = 'APIKEYGOESHERE'  # API KEY
    handler = ipinfo.getHandler(access_token)
    # Gets all the data that the free version of the api gets.
    Ip_Info = handler.getDetails(ip_address)
    # Only grabbing the parts that I want.
    IPGeoD = (Ip_Info.org, Ip_Info.country_name,  Ip_Info.region, Ip_Info.city)
    return IPGeoD
...
def ipexport(EData):
    for ipData in range(0, len(EData)):
        if EData[ipData][9] == None and "192." not in EData[ipData][0] and "172." not in EData[ipData][0]:  # External
        ...
        if EData[ipData][9] == None and "192." in EData[ipData][0] or "172." in EData[ipData][0]:  # Internal
